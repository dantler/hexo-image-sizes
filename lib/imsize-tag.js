...
function factory (hexo) {
  ...
  */
  ...
  return function imsizeTag (args, content) {
    ...
    //hexoRelativeResizedImagePath = Utility.pathShiftPrivateDirectory(hexoRelativeResizedImagePath);
    hexoRelativeResizedImagePath = Utility.pathToBackslashPath(hexoRelativeResizedImagePath);

    let inlineImageIsOriginal = (null === profileName);
    ...
    // If the original is present, either inline or linked, then dont remove it.
    if (linkImageIsOriginal || inlineImageIsOriginal) {
      debug("Original image is present. Dont delete route.");
    } else if (0) {
      // TODO: add a config flag to always keep image routes.
    } else {
      // Else remove original since it is unused
      debug(`Removing route to unused original: ${hexoRelativeInput}`);
      hexo.route.remove(hexoRelativeInput);
    }
